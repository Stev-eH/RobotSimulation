...
public class UR10Controller : MonoBehaviour
{
    ...
    [SerializeField]
    private float[] jointOffset = { 0f, 90f, 0f, 90f, 0f, 0f };
    ...
    {
        initializeJoints();
        solverEnabled = ikSolver.isActiveAndEnabled;
    }

    //https://rosettacode.org/wiki/Map_range#C
    private float remap(float a1, float a2, float b1, float b2, float s) => (b1 + (s - a1) * (b2 - b1) / (a2 - a1));
    ...
    void Update()
    {
        ...
        if (solverEnabled)
        {
            for (int i = 0; i < 6; i++)
            {
                // It works somehow
                if ((i == 0) | (i == 4))
                    jointValues[i] = ((remap(0, 360, -180, 180, jointList[i].transform.localEulerAngles.y) - 180 - jointOffset[i]) * jointSign[i] % 360);
                else
                    jointValues[i] = ((remap(0, 360, -180, 180, jointList[i].transform.localEulerAngles.x) - 180 - jointOffset[i]) * jointSign[i] % 360);
            }
        }
    }

    float calculateDiff(float value, float offset)
    {
        if (value + offset >= 180)
        {
            float diff = 180f - value;
            value = offset - diff;
            return -value;
        }
        else
    ...
    public GameObject[] GetJointList() { return jointList; }
}